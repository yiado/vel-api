...
class UserController extends APP_Controller {
    ...
    /**
     * Retorna los usuarios que no est�n asociados al grupo especicado por parametro.
    ...
    function preferences() {
        ...
        try {
            ...
            if (!empty($new_password) && !empty($new_password_cfrm)) {
                ...
                //	Comparar la coincidencia de la contrase�a actual con la ingresada por el usuario
                $credendital_user = Doctrine_Core :: getTable('User')->validate($user_username, $user_password);

                if (!empty($credendital_user->user_username)) {

                    // Comparar la coincidencia de la contrase�a nueva con la confirmada
